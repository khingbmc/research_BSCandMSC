                ...
                for k in range(len(save_weight[i][j]['weights'])):
                    ...
                    for c in range(num_outputs):
                        for a in np.arange(0.0, 1.1, 0.1):
                            sub_weight[c][round(c, 1)] = save_weight[i][j]['weights'][k]
...
# %% Model part
    ...
    for row in list_test:
        ...
        file_object = open('../../../present/learning_rate_test/BC/BPNN+MW0/result/BC_prediction_1'+'.txt', 'a')
