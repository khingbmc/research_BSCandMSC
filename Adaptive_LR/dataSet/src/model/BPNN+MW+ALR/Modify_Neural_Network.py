...
class NeuralModel:
    def __init__(self, number_input_layer, number_hidden_layer, number_output_layer, weight_input_hidden, weight_hidden_output, number_data_each_class):
        ...
        self.sum_error = []
        self.learn_rate = [0.1 for _ in range(number_output_layer)]
        self.save_learnrate = [[] for _ in range(number_output_layer)]
        self.a = 0
            ...
            if i != len(self.network) - 1:
                for j in range(len(layer)):
                    ...
                    for k in range(len(self.network[i + 1])):
                        ...
                        error += (1-(self.number_data_each_class[k]/sum(self.number_data_each_class)))*neuron['weights'][j] * neuron['errors']
                    errors.append(error)
            else:
                for j in range(len(layer)):
                    neuron = layer[j]
                    errors.append((1-(self.number_data_each_class[j]/sum(self.number_data_each_class)))*(expected[j] - neuron['output']))
            for j in range(len(layer)):
                neuron = layer[j]
            ...
            for neuron in self.network[i]:
                for j in range(len(inputs)):
                    ...
                    if i != 0:
                        ...
                        neuron['weights'][j][round(inputs[j], 1)] += self.learn_rate[self.num_class] * neuron['errors'] * inputs[j]
