...
# %% Init Weight in neural network model
    weight1 = [{'weights':[{'default': random()} for i in range(num_inputs)]} for i in range(number_node_hidden)]
...
# %% Model part
    ...
    for row in list_test:
        ...
        file = open('../../../present/learning_rate_test/Diabetes/BPNN+ALR/results/Diabetes_prediction_1'+'.txt', 'a')
    ...
    file = open('../../../present/learning_rate_test/Diabetes/BPNN+ALR/results/Diabetes_prediction_1'+'.txt', 'a')
    file.write("Accuracy : "+ str(sum_accuracy))
    file.close()
